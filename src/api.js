...
class LineAPI {
  ...
  _qrCodeLogin() {
    ...
    this._client.getAuthQrcode(true, 'WawanUye-PC',(err, result) => {
        ...
        unirest.get('https://gd2.line.naver.jp/Q')
          ...
          .end(async (res) => {
			...
			this._authService.loginZ(reqx,(err,success) => {
                ...
                this.options.path = this.config.LINE_COMMAND_PATH;
                this.setTHttpClient(this.options);
			    this.options.headers['User-Agent'] = 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.84 Safari/537.36';
			    this.axz = true;
			    this.setTHttpClient(this.options);
			    this.axz = false;
